...
StackWarden itself uses Trunk Based Development: `main` is the only long-lived branch, topic branches are short-lived, and commits and PR titles follow Conventional Commits. See the repository [CONTRIBUTING.md](../../CONTRIBUTING.md). Install the repository's local branch and commit-message guardrails with `bun run hooks:install` from the repository root.
...
```bash
bun run --filter stackwarden docs:generate
bun run --filter stackwarden docs:check
bun run --filter stackwarden projections:check
bun run --filter stackwarden config:schema:check
```
